...
<body>
 ...
 <?php if(isset($cate_rows)): ?></div><?php endif; ?> 
    ...
    <div class="main-title">
        <h2><?php if(isset($data)): ?>[ <?php echo ($data); ?> ] 子<?php endif; ?>分类管理 </h2>
    </div>

    <div class="cf">
        <a class="btn" href="<?php echo U('addTypeTwo',array('pid'=>I('get.pid')));?>">新 增</a>
        ...
        <!-- <div class="search-form fr cf">
            <div class="sleft">
                ...
                <a class="sch-btn" href="javascript:;" id="search" url="/code/index.php?s=/Admin/GoodsCategory/goodsTypeTwo/pid/1.html"><i class="btn-search"></i></a>
            </div>
        </div> -->
    </div>
        ...
        <form class="ids">
            <table>
				...
				<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$menu): $mod = ($i % 2 );++$i;?><tr>
                        <td><input class="ids row-selected" type="checkbox" name="id[]" value="<?php echo ($menu["goodstype_id"]); ?>"></td>
                        <td><?php echo ($menu["goodstype_id"]); ?></td>
                        <td><img style="width: 40px;height: 40px;" src="/code<?php echo ($menu["path"]); ?>"></td>
                        <td>
                            <?php if($menu['pid'] == 0): ?><!-- 一级分类 -->
                               <a href="<?php echo U('goodsTypeTwo?pid='.$menu['goodstype_id']);?>"><?php echo ($menu["typename"]); ?></a>
                            <?php else: ?>
                                <!-- 二级分类 -->
                                <?php echo ($menu["typename"]); endif; ?>
                        </td>
                        <td><?php if(($menu["pid"] == 0)): ?>无 <?php else: ?> <?php echo ($data); endif; ?></td>
                        <td>
                            <a title="编辑" href="<?php echo U('addTypeTwo?pid='.$pid.'&goodstype_id='.$menu['goodstype_id']);?>">编辑</a>
                            <a class="confirm ajax-get" title="删除" href="<?php echo U('deltype?goodstype_id='.$menu['goodstype_id']);?>">删除</a>
                        </td>

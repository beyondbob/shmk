 ...
 <?php if(isset($cate_rows)): ?></div><?php endif; ?> 
		...
		<div class="search-form fr cf">
			...
			<div class="drop-down">
				<span id="sch-sort-txt" class="sort-txt" data="<?php echo ($status); ?>"> 
                    <?php switch($status): case "1": ?>已签收<?php break;?>
				...
				<ul id="sub-sch-menu" class="nav-list hidden">
					...
					<li><a href="javascript:;" value="1" onclick="select('1');">已签收</a></li>
                    <li><a href="javascript:;" value="2" onclick="select('2');">已发货</a></li>
                    <li><a href="javascript:;" value="4" onclick="select('4');">待发货</a></li>
				</ul>
			</div>
				<input type="text" name="selvalue" class="search-input" value="<?php echo I('selvalue');?>" placeholder="输入零售单号">
            ...
            <div class="btn-group-click adv-sch-pannel fl">
                ...
                <div class="dropdown cf">
                	...
                	<div class="row">
                		<label>买家：</label>
                		<input type="text" name="account-name" class="text input-2x" value="" placeholder="请输入买家姓名">
                	</div>
                </div>
            </div>
		</div>
    ...
    <tbody>
		<?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
            <td><a href="<?php echo U('PurchaseOrder/edit?purorder_id='.$vo['purorder_id']);?>"><?php echo ($vo["purorder_no"]); ?></a></td>
            <td><img width="35px" src='<?php echo ($vo["avatar"]); ?>'/> <?php echo ($vo["account_name"]); ?></td>
            <td><?php echo ($vo["account_tel"]); ?></td>
            <td><?php echo ($vo["shop_name"]); ?></td>
            <td><?php echo ($vo["starttime"]); ?></td>
            <td><?php echo ($vo["finishtime"]); ?></td>
			<td><?php switch($vo["status"]): case "1": ?>已签收<?php break;?>
                    <?php case "2": ?>已发货<?php break;?>
                    <?php case "4": ?>待发货<?php break;?>
                    <?php default: endswitch;?></td>
            <td><a href="<?php echo U('PurchaseOrder/edit?purorder_id='.$vo['purorder_id']);?>">查看</a></td>
...
<script status="text/javascript" src="/shmk/Public/static/datetimepicker/js/locales/bootstrap-datetimepicker.zh-CN.js" charset="UTF-8"></script>
    ...
    //导航高亮
    highlight_subnav('<?php echo U('PurchaseOrder/index');?>');
	</script>

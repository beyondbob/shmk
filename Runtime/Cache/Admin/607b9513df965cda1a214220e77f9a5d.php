...
<body>
 ...
 <?php if(isset($cate_rows)): ?></div><?php endif; ?> 
	...
	<!-- tab切换 -->
    <ul class="tab-nav nav distributionTab">
	<li><a href="<?php echo U('type');?>">经销商头衔</a></li>
	<li><a href="<?php echo U('integral');?>">月度积分结算配置</a></li>
	<li><a href="<?php echo U('starGradeMonth');?>">月度星级规则配置</a></li>
	<li><a href="<?php echo U('yearIntegral');?>">年度积分结算配置</a></li>
	<li><a href="<?php echo U('starGradeYear');?>">年度星级规则配置</a></li>
</ul>
<script type="text/javascript">
	var url = window.location.href;
	var arr = url.split("?s=");
	//console.log(arr[1])
	if( arr[1] == '/Parameter/type.html' || arr[1] == '/Admin/Parameter/type.html'){
		$(".distributionTab li:eq(0)").addClass("tab current")
	}
	if( arr[1] == '/Parameter/integral.html' || arr[1] == '/Admin/Parameter/integral.html' ){
		$(".distributionTab li:eq(1)").addClass("tab current")
	}
	if( arr[1] == '/Parameter/starGradeMonth.html' || arr[1] == '/Admin/Parameter/starGradeMonth.html' ){
		$(".distributionTab li:eq(2)").addClass("tab current")
	}
	if( arr[1] == '/Parameter/yearIntegral.html' || arr[1] == '/Admin/Parameter/yearIntegral.html' ){
		$(".distributionTab li:eq(3)").addClass("tab current")
	}
	if( arr[1] == '/Parameter/starGradeYear.html' || arr[1] == '/Admin/Parameter/starGradeYear.html' ){
		$(".distributionTab li:eq(4)").addClass("tab current")
	}
    ...
    <thead>
		...
		<th class="">升级（元）</th>
		<th class="">员工升级（元）</th>
		<th class="">经销商有效期（月）</th>
		<th class="">员工经销商有效期（月）</th>
		<th class="">操作</th>
		</tr>
    </thead>
    <tbody>
		<?php if(!empty($typelist)): if(is_array($typelist)): $i = 0; $__LIST__ = $typelist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
           <!--  <td><input class="ids" type="checkbox" name="id[]" value="<?php echo ($vo["student_id"]); ?>" /></td> -->
            <td><?php echo ($vo["accounttype_id"]); ?></td>
            <td><?php echo ($vo["type_name"]); ?></td>
            <!-- <td><?php echo ($vo["buy_right_text"]); ?></td>
            <td><?php echo ($vo["sell_right_text"]); ?></td> -->
            <td><?php echo ($vo["up_money"]); ?></td>
            <td><?php echo ($vo["staff_upmoney"]); ?></td>
            <td><?php echo ($vo["expire_month"]); ?></td>
            <td><?php echo ($vo["staff_expire_month"]); ?></td>
			<td><a href="<?php echo U('Parameter/etype?accounttype_id='.$vo['accounttype_id']);?>">编辑</a></td>
    ...
    //导航高亮
    highlight_subnav('<?php echo U('Parameter/type');?>');
	</script>

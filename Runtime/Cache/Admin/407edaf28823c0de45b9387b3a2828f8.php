 ...
 <?php if(isset($cate_rows)): ?></div><?php endif; ?> 
		...
		<div class="search-form fr cf">
			...
			<div class="drop-down">
				<span id="sch-sort-txt" class="sort-txt" data="<?php echo ($status); ?>"> 
                    <?php switch($status): case "0": ?>未处理<?php break;?>
				...
				<ul id="sub-sch-menu" class="nav-list hidden">
					...
					<li><a href="javascript:;" value="0" onclick="select('0');">未处理</a></li>
                    <li><a href="javascript:;" value="1" onclick="select('1');">已处理</a></li>
				</ul>
			</div>
				<input type="text" name="selvalue" class="search-input" value="<?php echo I('selvalue');?>" placeholder="输入提现单号">
				<a class="sch-btn" href="javascript:;" id="search" url="<?php echo U('cashIndex');?>"><i class="btn-search"></i></a>
			</div>
            <div class="btn-group-click adv-sch-pannel fl">
                ...
                <div class="dropdown cf">
                	...
                	<div class="row">
                		<label>提现人：</label>
                		<input type="text" name="account-name" class="text input-2x" value="" placeholder="输入提现人姓名">
                	</div>
                </div>
            </div>
		</div>
    ...
    <tbody>
		<?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
            <td><a href="<?php echo U('WorkOrders/cashEdit?id='.$vo['id']);?>"><?php echo ($vo["work_no"]); ?></a></td>
            <td><img width="45px" src='<?php echo ($vo["avatar"]); ?>'/> <?php echo ($vo["name"]); ?></td>
            <!-- <td><?php echo ($vo["bank"]); ?></td>
            <td><?php echo ($vo["cardnum"]); ?></td> -->
            <td>￥ <?php echo ($vo["charge"]); ?></td>
            
            <td><?php switch($vo["type"]): case "1": ?>手动提现<?php break;?>
                    <?php case "3": ?>自动提现<?php break;?>
                    <?php default: endswitch;?></td>
			<td><?php switch($vo["status"]): case "0": ?>未处理<?php break;?>
                    <?php case "1": ?>已处理<?php break;?>
                    <?php default: endswitch;?></td>
                <td><?php echo ($vo["launchtime"]); ?></td>
            <td><?php echo ($vo["responsetime"]); ?></td>
            <td><a href="<?php echo U('WorkOrders/cashEdit?id='.$vo['id']);?>">查看</a></td>
...
<script status="text/javascript" src="/shmk/Public/static/datetimepicker/js/locales/bootstrap-datetimepicker.zh-CN.js" charset="UTF-8"></script>
    ...
    //导航高亮
    highlight_subnav('<?php echo U('WorkOrders/cashIndex');?>');
	</script>

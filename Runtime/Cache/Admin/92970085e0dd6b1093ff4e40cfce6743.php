        ...
        <div class="search-form fr cf">
            ...
            <!-- <div class="drop-down">
                ...
                <ul id="sub-sch-menu" class="nav-list hidden">
                    ...
                    <?php if(is_array($years)): foreach($years as $key=>$vo): ?><li><a href="javascript:;" value="" onclick="select('<?php echo ($vo["year"]); ?>');"><?php echo ($vo["year"]); ?></a></li><?php endforeach; endif; ?>
                </ul>
            </div> -->
            <div class="drop-down">
                <span id="sch-sort-txt" class="sort-txt" data="<?php echo ($year); ?>">
                ...
                <ul id="sub-sch-menu" class="nav-list hidden">
                    <li><a href="javascript:;" value="" onclick="select('');">全部</a></li>
                    <?php if(is_array($years)): foreach($years as $key=>$vo): ?><li><a href="javascript:;" value="<?php echo ($vo["year"]); ?>" onclick="select('<?php echo ($vo["year"]); ?>');"><?php echo ($vo["year"]); ?>年</a></li><?php endforeach; endif; ?>
                </ul>
            </div>
            <input type="text" name="selvalue" class="search-input" value="<?php echo I('selvalue');?>" placeholder="输入姓名或昵称">
            <a class="sch-btn" href="javascript:;" id="search" url="<?php echo U('yearBonusList');?>"><i class="btn-search"></i></a>
            </div>
        </div>
    </div>
        ...
        <form class="ids">
            <table>
                ...
                <?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><tr>
                        ...
                        <td><?php echo ($item["percent_one"]); ?></td>
                        <td><?php echo ($item["bonus_one"]); ?></td>
                        <td><?php echo ($item["integral_two"]); ?></td>
                        <td><?php echo ($item["percent_two"]); ?></td>
                        <td><?php echo ($item["bonus_two"]); ?></td>
                        <td><?php echo ($item["total_money"]); ?></td>
                        <td><?php echo ($item["total_money"]); ?></td>
                        <td>
                            <?php if($item['status'][''] == 0 AND $item['account_id'] != NULL): ?><a href="#" toid="<?php echo ($item["id"]); ?>" class="finish_bonus">确认汇款</a>
                            <?php else: endif; ?>
                        </td>
...
<script type="text/javascript">
    $(".finish_bonus").click(function(){
      ...
      $.messager.confirm('温馨提示', '你确定要执行这个操作?', function(r){
            if (r){
                 $.ajax({
                    method:"post",
                    url:"<?php echo U('');?>",
   ...
   highlight_subnav('<?php echo U('Integral/yearBonusList');?>');
</script>

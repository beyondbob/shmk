...
<body>
 ...
 <?php if(isset($cate_rows)): ?></div><?php endif; ?> 
    ...
    <div class="main-title">
        <h2><?php echo !empty($info['spec_id'])?'编辑':'新增';?>规格</h2>
    </div>
    <form action="<?php echo U();?>" method="post" class="form-horizontal">
        <table class="item-details">
            <tbody>
                <tr class="form-item">
                    <td class="row-left">类型名称</td>
                    <td class="row-right">
                        <input type="text" class="text input-large" name="title" value="<?php echo ((isset($type["title"]) && ($type["title"] !== ""))?($type["title"]):''); ?>">
                        <span class="check-tips"></span>
                    </td>
                </tr>
                <tr class="form-item">
                    <td class="row-left">上级规格</td>
                    <td class="row-right">
                        <select name="pid">
                            <?php if(is_array($Menus)): $i = 0; $__LIST__ = $Menus;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$menu): $mod = ($i % 2 );++$i; if($menu['spec_id'] == $type['pid']): ?><option value="<?php echo ($menu["spec_id"]); ?>" selected="selected">&nbsp;&nbsp;&nbsp;<?php echo ($menu["title"]); ?>&nbsp;&nbsp;</option>
                                <?php else: ?>
                                    <option value="<?php echo ($menu["spec_id"]); ?>">&nbsp;&nbsp;&nbsp;<?php echo ($menu["title"]); ?>&nbsp;&nbsp;</option><?php endif; endforeach; endif; else: echo "" ;endif; ?>
                        </select>
                        <span class="check-tips">（所属的上级类型）</span>
                    </td>
                </tr>
                <tr class="form-item">
                    <input type="hidden" name="spec_id" value="<?php echo ((isset($type["spec_id"]) && ($type["spec_id"] !== ""))?($type["spec_id"]):''); ?>">

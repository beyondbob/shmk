...
<body>
 ...
 <?php if(isset($cate_rows)): ?></div><?php endif; ?> 
    ...
    <div class="tab-content">
        <!-- 表单 -->
        <form action="<?php echo U('certificate');?>" class="form-horizontal" method="post" >
            <table class="item-details" width="100%">
                <tbody>
                <tr class="form-item">
                    <td class="row-left">内容</td>
                    <td class="row-right">
                        <textarea  class=" input-large" rows="4" name="desc"><?php echo ($desc["value"]); ?></textarea>
                        <span class="check-tips">({$name}代表姓名，{$brand}代表品牌，{$number}代表编号)</span>
                    </td>
                </tr>
                <tr class="form-item">
                    <td class="row-left">证书背景图</td>
                    <td class="row-right">
                        <input type="file" id="upload_picture">
                        <input type="hidden" name="icon" value="<?php echo ((isset($img["value"]) && ($img["value"] !== ""))?($img["value"]):''); ?>" class="icon" />
                        <div class="upload-img-box">
                            <?php if(!empty($img["path"])): ?><div class="upload-pre-item">

                                    <img src=".<?php echo ((isset($img["path"]) && ($img["path"] !== ""))?($img["path"]):$model['path']); ?>"
                                         data-id="<?php echo ((isset($img["id"]) && ($img["id"] !== ""))?($img["id"]):''); ?>" /> <span
                                        class='btn-close' title='删除图片' onclick='del(event);'></span>
                                </div><?php endif; ?>
                        </div>
                        <span class="check-tips">（请选择选择合适尺寸的背景图）</span>
                    </td>
                </tr>
    ...
    <script type="text/javascript">
        ...
        //导航高亮
        highlight_subnav('<?php echo U("certificate");?>');
    </script>

...
<body>
 ...
 <?php if(isset($cate_rows)): ?></div><?php endif; ?> 
	...
	<!-- tab切换 -->
	<ul class="tab-nav nav mallTab">
	<li ><a href="<?php echo U('postage');?>">邮费配置</a></li>
	<li ><a href="<?php echo U('sms');?>">短信接口</a></li>
	<li ><a href="<?php echo U('pay');?>">支付相关</a></li>
	<li ><a href="<?php echo U('copy');?>">版权信息</a></li>
</ul>
<script type="text/javascript">
	var url = window.location.href;
	var arr = url.split("?s=");
	console.log(arr[1]);
	if( arr[1] == '/Parameter/postage.html' || arr[1] == '/Admin/Parameter/postage.html' ){
		$(".mallTab li:eq(0)").addClass("tab current");
	}
	if( arr[1] == '/Parameter/sms.html' || arr[1] == '/Admin/Parameter/sms.html'){
		$(".mallTab li:eq(1)").addClass("tab current");
	}
	if( arr[1] == '/Parameter/pay.html' || arr[1] == '/Admin/Parameter/pay.html'){
		$(".mallTab li:eq(2)").addClass("tab current");
	}
	if( arr[1] == '/Parameter/copy.html' || arr[1] == '/Admin/Parameter/copy.html'){
		$(".mallTab li:eq(3)").addClass("tab current");
	}
    ...
    <tbody>
			<?php if(!empty($settings)): ?><form action="<?php echo U('save');?>" class="form-horizontal" method="post" >
				<?php if(is_array($settings)): $k = 0; $__LIST__ = $settings;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><div class="form-item cf">
						<tr>
				           <!--  <td><input class="ids" type="checkbox" name="id[]" value="<?php echo ($vo["student_id"]); ?>" /></td> -->
				            <td><?php echo ($vo["title"]); ?></td>
				            <td>
				            	<input type="radio" name="<?php echo ($vo['key']); ?>-value" value="1" <?php if($vo["value"] == 1): ?>checked="true"<?php endif; ?> >启用
				            	<input type="radio" name="<?php echo ($vo['key']); ?>-value" value="0" <?php if($vo["value"] == 0): ?>checked="true"<?php endif; ?> >禁用
				            	<!-- <input type="text" class="text input-large" name="<?php echo ($vo['key']); ?>-value" value="<?php echo ($vo["value"]); ?>"> -->
				            </td>
				            <input type="hidden" name="key[]" value="<?php echo ($vo['key']); ?>">
							<!-- <td>
								<button class="btn submit-btn ajax-post hidden" id="submit" type="submit" target-form="form-horizontal-<?php echo ($k); ?>">	保存
								</button>
							</td> -->
						</tr>
	...
	<button class="btn ajax-post" id="submit" type="submit" target-form="form-horizontal">保存</button>
    <div class="page">
        <?php echo ($_page); ?>
    </div>
    ...
    //导航高亮
    highlight_subnav('<?php echo U('Parameter/sms');?>');
	</script>

    ...
    <tbody>
		<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
           <!--  <td><input class="ids" type="checkbox" name="id[]" value="<?php echo ($vo["student_id"]); ?>" /></td> -->
			...
			<td>
				<?php if($vo["status"] == 1): ?><a class="ajax-get confirm" href="<?php echo U('AuthManager/onoff?status=0&id='.$vo['id']);?>">禁用 </a>|
				<?php else: ?>
					<a class="ajax-get" href="<?php echo U('AuthManager/onoff?status=1&id='.$vo['id']);?>">启用 </a>|<?php endif; ?>
				<a href="<?php echo U('AuthManager/editAdImg?id='.$vo['id']);?>">编辑</a>
			</td>
